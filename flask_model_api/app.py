...
def receive_symptoms():
    try:
        data = request.get_json()
        if not data or 'symptoms' not in data:
            logger.error("Missing symptoms data")
            return jsonify({"error": "Missing symptoms data"}), 400
            
        # Use the existing predict function's logic
        input_symptoms = np.array([data['symptoms']])
        validate_input_symptoms(data['symptoms'])
        
        # Load required components and make prediction
        model, label_encoder = load_model_and_encoder()
        finaldiseases, singledrugeffect, druginteraction = load_drug_data()

        # Process prediction and update the UI
        return predict()
