...
# Initialize Gemini
genai.configure(api_key='AIzaSyCh1kHiLstAiz9u98ua-GKMPW_b8ARCoIM')
...
supabase = create_client(
    ...
    'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVwZ2pzd3NwbnZhYmVreW5xaWtkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Mzk2NDI1NzUsImV4cCI6MjA1NTIxODU3NX0.Fmnq7YOlIhQfTNlUjKwKvPTsVFgYwwFf60f0TPuvivo'
)
...
def analyze_chat():
    ...
    symptoms_binary = convert_to_binary_array(detected_symptoms)

    # Try to send to port 3000 with multiple retries
    max_retries = 3
    for attempt in range(max_retries):
        try:
            response = requests.post(
                'http://localhost:3000/receive-symptoms', 
                json={'symptoms': symptoms_binary},
                timeout=2  # Reduced timeout
            )
            if response.status_code == 200:
                print("Successfully sent to port 3000")
                break
        except requests.exceptions.RequestException as e:
            print(f"Attempt {attempt + 1} failed: {e}")
            if attempt == max_retries - 1:
                print("All attempts to send to port 3000 failed")
            else:
                time.sleep(1)  # Wait 1 second before retrying

    # Save detected symptoms regardless of port 3000 status
    ...
    # Return success even if port 3000 failed - the user will see results there when it's available
    return jsonify({
        "success": True, 
        "detected_symptoms": detected_symptoms,
        "port_3000_status": "sent" if response.status_code == 200 else "failed"
    })
